...
LightND 是一款基于 **PySide6** 构建的 Windows 桌面应用程序，面向轻小说读者，提供从在线书库下载、文本格式化、电子书转换到阅读进度管理的完整工作流。
...
```python
self.bb_param = ['', '', [1, '+'], '']  # [genre, bunko, [order, sign], search_key]
self.bb_liked = 0                        # 0=全部, 1=收藏, 2=未收藏
self.bw_list: list[BkWt]                # 全部书籍卡片列表
```
...
refresh_bw_list()
    ...
    └── render_book_bank(process_bw_list())  # 重新渲染
```
...
| 4 | NovelSearcher | 小说搜索（需 ENABLE_ISF） | 1068×640 |
...
1. `render_book_bank()` 创建 `WidgetGrid`，设置 3 列网格，计算总高度
2. 仅前 15 本书调用 `Initialize()` 完成完整渲染
3. 其余书作为占位控件加入网格
...
| v8.3 | — | 多线程启动（启动加速最高 50%）、细节菜单 UI 翻新 |
| v8.2.1 | — | 书库动态渲染（启动加速 60%+）、ENABLE_BANK 配置增强 |
| v8.2 | — | 代理端口配置（PROXY_PORT）、配置项整型范围限制 |
| v8.1 | — | RMZ 后缀模板、文本格式化切片功能 |
| v8.0 | — | 书籍结构化重整、SIMPLE_BANK_FILE 配置、入库时间排序 |
| v7.5 | — | 多任务菜单、导入多任务功能 |
